...
int main(int argc,char** argv)
{
  ...
  /*read one ring at a time from input file. Main loop */
	while(( arc_ptr = KNTIOread_linear_lammps_version( fp_in_data)) != NULL )
	{
    ...
    int cls_points = closed_arc.len - arc_ptr->len;
		//double axis[3]={0.01,0.1,0.8};
		KNTLrotate_random(&closed_arc);

    buffer_init (&COORD_BUFFER,closed_arc.len );
    //knt_rect = KNTLrectify_coord_local ( &closed_arc,0,arc_ptr->len-1, min_stride,max_stride );
		int border= (3*max_stride<arc_ptr->len)? max_stride:arc_ptr->len/3;
    knt_rect = KNTLrectify_coord_local ( &closed_arc,border,arc_ptr->len-border, min_stride,max_stride );
		//
		if (flag_debug)
		{
			fprintf(stderr,"--------------\n");
		...
		//--Begin the search
		st_p=0;
		end_p=knt_rect->len-cls_points-1;
		for (i = 0 ; i< N_SEARCHES ; i++ )
		{
			if ( S_flag[i] )
			{
		...
		if ( k_id != K_Un )
		{
			if (S_flag[BU] )
			{
				KNTLloc_shrt_knot_rect2    ( &closed_arc, knt_rect,st_p,end_p, KNTCqhull_hybrid_close_subchain, alx_wspace);
			}
			if ( S_flag[TD] )
			{
				KNTLloc_shrtcnt_knot_rect2 ( &closed_arc, knt_rect,st_p,end_p, KNTCqhull_hybrid_close_subchain, alx_wspace);
			}
			if ( S_flag[UNS] )
			{
				KNTLloc_shrt_knot_unsafe_rect2    ( &closed_arc, knt_rect,st_p,end_p, KNTCqhull_hybrid_close_subchain, alx_wspace);
			//	 KNTLloc_shrt_knot_unsafe_rect    ( knt_ptr, knt_rect, KNTCqhull_hybrid_close_subchain, alx_wspace);
			}
			knt_here = knt_rect;
			do
			{
				for(i=0;i<N_SEARCHES;i++)
				{
					if(knt_here->arc_type == arc_id[i])
					{
						...
						sprintf(line[i],"%s %d %d %d \t",
								line[i], start,end,(end > start)? end - start: end-start+ closed_arc.len);
						if (flag_debug)
						{
							fprintf(stderr,"--------------\n");
							KNTIOprint_arc(stderr,&closed_arc,start,end);
							fprintf(stderr,"--------------\n");
						}
					}
				}
